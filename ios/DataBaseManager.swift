...
class DatabaseManager {
    ...
    // Aqui no banco chamamos de fakeConf/realConf, mas no código usaremos confidenceFake/confidenceReal
    ...
    func addMessage(sessionId: String, text: String, isUser: Bool, label: String? = nil, confidenceFake: Double? = nil, confidenceReal: Double? = nil) throws -> ChatMessage {
        guard let db = db else { throw DatabaseError.connectionFailed }
        ...
        let insert = messagesTable.insert(
            ...
            timestamp <- now,
            self.label <- label,
            fakeConf <- confidenceFake,
            realConf <- confidenceReal
        )
        try db.run(insert)
        
        return ChatMessage(id: newId, sessionId: sessionId, text: text, isUser: isUser, timestamp: now, label: label, confidenceFake: confidenceFake, confidenceReal: confidenceReal)
    }
